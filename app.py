...
if st.button("Predict Risk Level"):
    # Susun data dalam DataFrame (Mesti sama susunan dengan training)
    data = pd.DataFrame([[age, bmi, smoking, crp, il6, vcam, glutathione, lipid, renal, liver]], 
                        columns=['Age', 'BMI', 'Smoking_Status', 'CRP', 'IL_6', 'VCAM_1', 'Glutathione', 'Lipid_Profile', 'Renal_Profile', 'Liver_Profile'])
    ...
    risk = prediction[0]
    if risk == 'High':
        st.error(f"Hasil: Risiko {risk}")
    elif risk == 'Medium':
        st.warning(f"Hasil: Risiko {risk}")
    else:
        st.success(f"Hasil: Risiko {risk}")
